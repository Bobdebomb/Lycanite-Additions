...
public class entitymycoflawg extends RideableCreatureEntity {
    ...
    //                    Constructor
    // ==================================================
    public entitymycoflawg(World world) {
        super(world);
        ...
        if(this.leapedAbilityReady && this.onGround && !this.getEntityWorld().isRemote) {
            ...
            List<EntityLivingBase> possibleTargets = this.getEntityWorld().getEntitiesWithinAABB(EntityLivingBase.class, this.getEntityBoundingBox().grow(distance, distance, distance), possibleTarget -> {
                if (!possibleTarget.isEntityAlive()
                        || possibleTarget == entitymycoflawg.this
                        || entitymycoflawg.this.isRidingOrBeingRiddenBy(possibleTarget)
                        || entitymycoflawg.this.isOnSameTeam(possibleTarget)
                        || !entitymycoflawg.this.canAttackClass(possibleTarget.getClass())
                        || !entitymycoflawg.this.canAttackEntity(possibleTarget))
                    return false;
