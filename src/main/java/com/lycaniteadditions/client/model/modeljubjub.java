...
public class modeljubjub extends ModelTemplateCocken {
    ...
    public modeljubjub(float shadowSize) {
        // Load Model:
        this.initModel("jubjub", LycanitesMobs.modInfo, "entity/jubjub");
    ...
    public void animatePart(String partName, EntityLiving entity, float time, float distance, float loop, float lookY, float lookX, float scale) {
        ...
        if(entity != null && !entity.onGround) {
            ...
            if (partName.equals("wingleftupper")) {
                this.rotate(80F, -(float)Math.toDegrees(flightLoop * 0.7), -0.385F);
                this.scale(1, 1, -1);
            }
            if (partName.equals("wingrightupper")) {
                this.rotate(80F, (float)Math.toDegrees(flightLoop * 0.7F), 0.385F);
                this.scale(1, 1, -1);
            }
